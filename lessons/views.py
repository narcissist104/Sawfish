...
def Invoice(request):
    requests = Request.objects.filter(Student_name=request.first_name)
    requests = Request.objects.filter(bank_id=request.bank.id)
    requests = Request.objects.filter(school_Name=request.school_Name)
    requests = Request.objects.filter(Student_Email=request.email   )
    requests = Request.objects.filter(Student_id=request.student.id   )
    requests = Request.objects.filter(school_address=request.school_address)
    return render(request, 'Invoice.html',{'requests':requests})

def View_balance (request):

    requests = Request.objects.filter(Student_name=request.first_name)
    requests = Request.objects.filter(bank_id=request.bank.id)
    requests = Request.objects.filter(student_payment=request.student_payment)
    requests = Request.objects.filter(Student_Email=request.email)
    requests = Request.objects.filter(school_Balance=request.school_Balance)
    return render(request, 'Invoice.html',{'requests':requests})
